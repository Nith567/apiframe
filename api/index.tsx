...
app.frame("/", (c) => {
  return c.res({
    ...
    intents: [
      ...
      <Button.Link href="https://passport.talentprotocol.com/signin">
        Register
      </Button.Link>,
    ],
  });
...
app.frame("/my-passport", farcasterDataMiddleware, async (c) => {
  ...
  const SHARE_BY_USER = `${baseUrl}?text=${encodeURIComponent(
    text
  )}&embeds[]=${encodeURIComponent(embedUrlByUser)}`;
